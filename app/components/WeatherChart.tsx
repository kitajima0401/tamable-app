        ...
        <p key={index} className="text-gray-100 text-sm">
          ...
          <span className="font-semibold">
            {typeof entry.value === 'number'
              ...
              : entry.value}{' '}
            {UNIT_MAP[entry.dataKey] ? UNIT_MAP[entry.dataKey][unitSystem] : ''}
          </span>
        </p>
      ))}
    </div>
...
export default function WeatherChart({
  ...
  const keys = useMemo(() => {
    ...
    metrics.forEach((m) => {
      if (period === 'daily') {
        if (m === 'temperature_2m') {
          result.push('temperature_2m_max', 'temperature_2m_min');
        } else if (m === 'precipitation') {
          result.push('precipitation_sum');
        } else if (m === 'windspeed_10m') {
          result.push('windspeed_10m_max');
        }
      } else {
        result.push(m);
      }
    ...
    <div className="w-full max-w-3xl mx-auto h-96 bg-gradient-to-br from-blue-950 via-indigo-950 to-blue-900 rounded-xl p-4 shadow-2xl border border-blue-800 overflow-visible flex justify-center">
      <ResponsiveContainer width="95%" height="100%">
        ...
        >
          ...
          {/* カスタムツールチップ */}
          <Tooltip
            content={<CustomTooltip period={period} unitSystem={unitSystem} />}
